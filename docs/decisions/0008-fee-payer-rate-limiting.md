...
Keying a budget on the **submitted** address has an accepted flip side: an attacker who knows a victim's address can spend that victim's issuing budget, denying them signup codes and recovery codes for the rest of the window. There is no way around it while the endpoint is unauthenticated — the address is the only identity available, and the per-IP windows alone would let anyone behind a shared egress deny everybody else. It is an argument for keeping the per-address limit no *tighter* than five: the looser it is, the more work the denial costs and the less likely a real user is to collide with it. Two things blunt it. The per-address window is charged **after** each route's existence check, so probing an address that does not exist — or is already registered — costs the owner nothing; and exceeding the signup or PIN reset attempt cap destroys the code rather than starting a timed lockout, so the remedy is always "ask for another", never "wait an hour" (see `src/lib/auth/verification-code.ts`).

The mechanism was extended again, by the security review of #18, to the three routes that take a one-time code *back* — `api/auth/verify-email`, `api/auth/pin/reset` (action `reset`) and `api/auth/recovery/verify`. Only the issuing side had been limited, which left the guessing side open: `api/auth/verify-email` had no limit of any kind. The real defence there is the per-code attempt cap in `src/lib/auth/store.ts`, now applied under `SELECT … FOR UPDATE` so that guesses issued in parallel cannot outrun it; these limits are depth on top of it, on their own `RATE_LIMIT_AUTH_VERIFY_*` budgets so that asking for a code and guessing at one do not spend each other.
