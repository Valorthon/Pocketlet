...
**The escrow expiry unit changes across the boundary, and the ledger is the source of truth.** The contract takes `expiry` as a **ledger sequence**; `claim_links.expiry` in Postgres is a **timestamp**. Both are stored — `expiry_ledger` is what the contract enforces, and `expiry` is *derived from it* at insert time in `api/wallet/claim-links/create/route.ts`. Do not recompute the timestamp from `expiryDays`: that was issue #137, and the two drifted by however long the user took to confirm plus whatever the range check allowed, so a sender could be refused a refund the chain would have permitted. The client bakes `expiryLedger` into the signed XDR at prepare time and must echo back **that** value, not a freshly read one — the route requires them to match exactly, and re-reading the ledger after the signing ceremony is why every claim link 400'd (issue #149). The staleness window is ~20 minutes (`MAX_PREPARE_AGE_LEDGERS`), not the day it used to be. `expiry_ledger` is nullable only because rows predating the fix have none. The conversion still assumes a 5-second ledger, which compounds — at 5.5s a 30-day link's timestamp is three days early, and the routes that gate on `expiry` would answer 410 while the contract would still release the funds. Tracked as issue #154; the fix is to compare ledgers at read time and treat the timestamp as display-only.

**Escrow operations are signed by the passkey, and the compiler cannot tell you otherwise.** The device key is registered with a `SignerLimits` map naming exactly the two SAC token contracts (`src/lib/wallet/device-key.ts`), and `api/wallet/device-key/submit` enforces that allowlist server-side. passkey-kit's `__check_auth` requires **every** requested auth context to be covered by some permitted signer. The escrow contract's `deposit` calls `sender.require_auth()` and `refund` calls `deposit.sender.require_auth()`, so each asks for a context on the *escrow* contract — which that map does not cover, and the wallet answers `MissingContext` (error 110). A claim-link deposit signed with `kit.sign(tx, deviceSigner)` therefore typechecks, lints, passes CI and fails on chain; it takes `kit.sign(tx)`, a passkey ceremony, instead (issue #148). A plain SAC transfer is the opposite case — its only context is the token `transfer`, which the device key *is* permitted to authorize — so `executeTransfer` keeps the device signer and must not be "made consistent" with the claim-link path. `claim` needs neither, having no `require_auth` at all. Both `send/page.tsx` call sites are pinned by a source scan in `src/lib/wallet/escrow-signing.test.ts`, which also asserts the exact contents of the limits map and of the server allowlist — widening either is the other way to make #148 go away, and it hands a PIN-protected 90-day `Temporary` signer authority over a third contract while doing nothing at all for users whose signer is already on chain. **`refund` is not pinned, because it has no call site yet**: `prepareEscrowRefundTx` is never called from `apps/web/src/app/`. Whoever builds the refund UI reintroduces this bug verbatim if they reach for the device key, and nothing will catch it — add a scan for it at the same time.
